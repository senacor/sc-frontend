export const classifications = [
  'Specialist',
  'Senior_Expert',
  'Senior_Management',
  'Lead_Expert',
  'Lead_Management'
];

export const dailyBusinessPerformanceLevels = [
  'scsheet.evaluation.dailyBusiness.1',
  'scsheet.evaluation.dailyBusiness.2',
  'scsheet.evaluation.dailyBusiness.3',
  'scsheet.evaluation.dailyBusiness.4',
  'scsheet.evaluation.dailyBusiness.5'
];

export const projectPerformanceLevels = [
  'scsheet.evaluation.dailyBusiness.1',
  'scsheet.evaluation.dailyBusiness.2',
  'scsheet.evaluation.dailyBusiness.3',
  'scsheet.evaluation.dailyBusiness.4',
  'scsheet.evaluation.dailyBusiness.5'
];

export const workEffectivityPerformanceLevels = [
  'scsheet.evaluation.workEffectivity.1',
  'scsheet.evaluation.workEffectivity.2',
  'scsheet.evaluation.workEffectivity.3',
  'scsheet.evaluation.workEffectivity.4',
  'scsheet.evaluation.workEffectivity.5'
];

export const workQualityPerformanceLevels = [
  'scsheet.evaluation.workQuality.1',
  'scsheet.evaluation.workQuality.2',
  'scsheet.evaluation.workQuality.3',
  'scsheet.evaluation.workQuality.4',
  'scsheet.evaluation.workQuality.5'
];

export const generalPerformanceLevels = [
  'scsheet.evaluation.general.1',
  'scsheet.evaluation.general.2',
  'scsheet.evaluation.general.3',
  'scsheet.evaluation.general.4',
  'scsheet.evaluation.general.5'
];

export const CATEGORY = {
  DAILY_BUSINESS: 'dailyBusiness',
  PROJECT: 'project',
  WORK_EFFICIENCY: 'workEfficiency',
  WORK_QUALITY: 'workQuality',
  SKILLS_IN_THE_FIELDS: 'skillsInTheFields',
  TEAM_IMPACT: 'impactOnTeam',
  COMPANY_IMPACT: 'impactOnCompany',
  SERVICE_QUALITY: 'serviceQuality',
  PERFORMANCE: 'performance',
  PR_CATEGORIES: 'prCategories'
};

export const SC_TAB = {
  EMPLOYEE: 'scEmployee',
  REVIEWER: 'scReviewer',
  MEETING: 'scheduleView'
};

export const SC_STATUS = {
  WITH_PR: 'SC_WITH_PR_CHOSEN',
  WITHOUT_PR: 'SC_WITHOUT_PR_CHOSEN',
  EMPLOYEE_SUBMITTED: 'EMPLOYEE_SUBMITTED',
  REVIEWER_SUBMITTED: 'REVIEWER_SUBMITTED',
  EMPLOYEE_MEETING_ACCEPTED: 'EMPLOYEE_MEETING_ACCEPTED',
  REVIEWER_MEETING_ACCEPTED: 'REVIEWER_MEETING_ACCEPTED',
  MEETING_CONFIRMED: 'MEETING_CONFIRMED'
};
